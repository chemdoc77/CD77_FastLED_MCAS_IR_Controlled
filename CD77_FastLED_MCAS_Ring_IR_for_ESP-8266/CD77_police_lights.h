#ifndef CD77_police_lights_h
#define CD77_police_lights_h
...
void cd77_police_lights_all(int carray, int segments, int wait){
  EVERY_N_MILLIS_I( police_time, 500){ 
  police_time.setPeriod( wait);
  x++;
  if (x >=  NUM_LEDS) {x = 0;}
...
void cd77_police_lights_all_random(uint8_t cycles,int wait){
    ...
    array_time.setPeriod (wait*(NUM_LEDS*cycles));
